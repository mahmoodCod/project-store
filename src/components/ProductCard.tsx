...
export default function ProductCard({ product, viewMode }: ProductCardProps) {
          ...
          <div className="flex-1 p-2 sm:p-3 md:p-4">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <Link href={`/product/${product.id}`} className="block">
                  <h3 className="text-xs sm:text-sm md:text-base font-semibold text-gray-900 mb-1 sm:mb-2 transition-colors line-clamp-2">
                    {product.name}
                  </h3>
                </Link>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end space-y-1">
                    ...
                    {product.discount && (
                      <div className="bg-red-500 text-white px-2 py-1 rounded text-xs font-bold shadow-lg">
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100 h-full flex flex-col">
      <div className="relative">
        <Link href={`/product/${product.id}`} className="block">
          {imageError ? (
            ...
            <Image
              ...
              width={400}
              height={192}
              className="w-full h-32 sm:h-40 md:h-48 object-cover transition-transform duration-200"
              onError={handleImageError}
            />
          )}
        </Link>
        ...
        {product.discount && (
          <div className="absolute top-12 right-1 sm:top-14 sm:right-2 bg-red-500 text-white px-2 py-1 rounded text-xs font-bold shadow-lg">
            تخفیف {product.discount.percentage}%
          </div>
        )}
      </div>
