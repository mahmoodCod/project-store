...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isForgotPassword) {
      ...
      if (!isValidEmailOrPhone(formData.email)) {
        alert('لطفاً یک ایمیل معتبر یا شماره موبایل معتبر وارد کنید');
        return;
      }
      ...
      console.log('Setting new password for:', formData.email);
  ...
  return (
    <div 
      className="fixed inset-0 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 bg-opacity-90 flex items-center justify-center z-50 p-4"
      onClick={handleClose}
    >
      <div 
        className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-white/20"
      ...
      >
        ...
        {!showSuccessMessage && (
          <form onSubmit={handleSubmit} className="p-6 space-y-6 bg-gradient-to-b from-white/50 to-gray-50/30">
            ...
            {isForgotPassword && showVerificationCode && !showNewPasswordForm && (
              <div>
            ...
            {/* New Password Form */}
            {isForgotPassword && showNewPasswordForm && (
              <>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    {formData.email.includes('@') ? (
                      <Mail className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-indigo-500" />
                    ) : formData.email.match(/^\d/) ? (
                      <Phone className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-indigo-500" />
                    ) : (
